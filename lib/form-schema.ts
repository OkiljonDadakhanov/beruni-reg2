...
export const teamLeaderSchema = z.object({
  ...
  passport_scan: z.any().optional(),
  id_photo: z.any().optional(),
})

// Define the contestant schema
export const contestantSchema = z.object({
  full_name: z.string().min(1, "Contestant name is required"),
  date_of_birth: z.date({ required_error: "Date of birth is required" }).optional(),
...
export const formSchema = z.object({
  ...
  total_accompanying_persons: z.string().min(1, "Number of persons is required"),
  team_leaders_count: z.string().min(1, "Number of team leaders is required"),
  team_leaders: z.array(teamLeaderSchema).min(1, "At least one team leader is required"),
